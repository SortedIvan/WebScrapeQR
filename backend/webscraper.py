...
def GetProcessedListings(city, priceLow, priceHigh):
    ...
    for i in range(len(rawListsings)):
        for raw in rawListsings[i]:
            ...
            interior = raw.find("li", attrs = {"class": "illustrated-features__item illustrated-features__item--interior"})
            processedListings.append(
                ParariusListing(url=url, houseLocation=houseLocation, houseName=houseName, housePrice=housePrice, houseType=1, houseMetrics=houseMetrix, nrOfRooms=nrOfRooms, interior=interior)
            )
